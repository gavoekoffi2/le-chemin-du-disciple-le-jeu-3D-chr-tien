...
GAME.Character = (function () {
  ...
  function loadRealPlayer(cb) {
    ...
    try {
      ...
      new THREE.GLTFLoader().parse(buf, '', gltf => {
        ...
        const ch = { group, inner, model, mixer, actions, bones, isReal: true, current: null, lastT: null, parts: {} };
        setAction(ch, 'idle');
        cb(ch);
  ...
  function addArmorPiece(ch, id) {
    ...
    if (ch.isReal) {
      ...
      if (id === 'ceinture') {
        ...
        m = new THREE.Mesh(new THREE.SphereGeometry(0.135, 14, 10, 0, Math.PI * 2, 0, Math.PI * 0.55), GAME.mat(0xc9a227));
        attachToBone(ch, 'Head', m, [0, 0.115, 0.015]);
      } else if (id === 'epee') {
        m = new THREE.Group();
  ...
  return { create, animate, addArmorPiece, createSheep, loadRealPlayer };
})();
